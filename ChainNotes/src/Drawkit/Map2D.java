...
// --Recycle Bin STOP (7/10/03 3:27 PM)
	...
	public Map2D(int PointInit, int pointsPerLine, int pointTotal, int shotPointSpacing, int receiverInterval, int pointType)
	{
	...
	public void paintComponent(Graphics g)
	{
		...
		int rowIndex;
		/* arbitrary counter to keep track of current point in row */
		int currPoint;
		...
		*/
		...
		for (rowIndex=0; rowIndex<3; rowIndex++)
		{
			...
			if (pointRemain > 0)
			{
				...
				{
					/* draw only the remaining points */
					pointsPerLine2 = pointRemain;
				}

				/* draw points for first mapped line */
				for (currPoint = 0; currPoint < pointsPerLine2; currPoint++)
				{
	...
	public Image rotateText(String s, FontMetrics fm, Font font, Color bg, Color fg)
	{
		...
		/* array to hold first Image's information */
		int[] pixels1;
		/* array to hold second Image's information */
		int[] pixels2;
		/* non-rotated image */
		Image img1;
		/* rotatated image */
		...
		/* to have labels that are not rotated, comment out from this try
		   to end of return img2 and return img1 instead */
		try
		{
			pg.grabPixels();
